...
void print_char_8x12(char c){ //this function will print the characters from font 8x12
  c -= 0x20;
  for(char row = 0; row < 12; row ++){
    unsigned short rowBits = font_8x12[c][row];
    for(char col = 0; col < 16; col++){
      unsigned short colMask = 1 << (15-col); //mask to select bit assoiciated with bit
      putchar((rowBits & colMask) ? '*' : ' ');
    }
    putchar('\n');
  }
